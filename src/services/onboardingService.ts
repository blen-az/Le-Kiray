...
/**
 * Approve agent account (Admin only)
 * This sets isApproved to true so they can publish listings.
 */
...
export const getAgentInvite = async (agentId: string): Promise<AgentInvite | null> => {
 ...
 const invites = querySnapshot.docs.map(doc => doc.data() as AgentInvite);
