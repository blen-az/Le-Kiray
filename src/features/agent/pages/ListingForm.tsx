...
const listingSchema = z.object({
 ...
 weeklyRate: z.coerce.number().optional().or(z.literal('')),
 ...
 if (!data.dailyRate || Number(data.dailyRate) <= 0) {
 ctx.addIssue({
 code: z.ZodIssueCode.custom,
 message: 'Daily rate is required for cars and vans',
 path: ['dailyRate'],
 });
 }
 }
...
const ListingForm: React.FC = () => {
 ...
 const isEarthMoving = category === VehicleCategory.EARTH_MOVING;

 // Cloudinary init is now fully lazy (on-click) inside handleImageUpload

 // loadListing logic moved to useQuery above
 ...
 const submitData = { 
 ...data, 
 dailyRate: data.dailyRate === '' ? undefined : Number(data.dailyRate), 
 weeklyRate: data.weeklyRate === '' ? undefined : Number(data.weeklyRate),
 ...
 <input {...register('make')} placeholder="e.g. Toyota, CAT" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:border-brand-main outline-none text-sm" />
 ...
 <input {...register('model')} placeholder="e.g. Land Cruiser" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:border-brand-main outline-none text-sm" />
 ...
 <div>
 <label className="text-xs font-black text-slate-500 uppercase tracking-widest block mb-2">Daily Rate (ETB) *</label>
 <input type="number" {...register('dailyRate')} placeholder="e.g. 4500" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:border-brand-main outline-none text-sm" />
 {errors.dailyRate && <p className="text-red-500 text-xs mt-1">{errors.dailyRate.message}</p>}
 </div>
 <div>
 <label className="text-xs font-black text-slate-500 uppercase tracking-widest block mb-2">Weekly Rate (ETB)</label>
 <input type="number" {...register('weeklyRate')} placeholder="Optional discount rate" className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:border-brand-main outline-none text-sm" />
 </div>
 </div>
 )}
 ...
 <textarea {...register('description')} rows={4} placeholder="Describe the vehicle, its condition, and key features..." className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:border-brand-main outline-none resize-none" />
 ...
 <img src={url} alt={`Vehicle ${i}`} className="w-full h-32 object-cover rounded-xl border border-slate-200 " />
