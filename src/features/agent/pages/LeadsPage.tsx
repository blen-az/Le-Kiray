...
const STATUS_OPTIONS: { 
  ...
  { 
    ...
    btnSelectedClass: 'bg-amber-600 text-white border border-amber-600',
    btnActionClass: 'bg-amber-500/10 text-amber-500 hover:bg-amber-500/20'
  },
  { 
    value: 'contacted', 
    label: 'Contacted', 
    ...
    btnSelectedClass: 'bg-blue-600 text-white border border-blue-600',
    btnActionClass: 'bg-blue-500/10 text-blue-500 hover:bg-blue-500/20'
  },
  { 
    value: 'quoted', 
    label: 'Quoted', 
    ...
    btnSelectedClass: 'bg-indigo-600 text-white border border-indigo-600',
    btnActionClass: 'bg-indigo-500/10 text-indigo-500 hover:bg-indigo-500/20'
  },
  { 
    value: 'won', 
    label: 'Accepted', 
    ...
    btnSelectedClass: 'bg-emerald-600 text-white border border-emerald-600',
    btnActionClass: 'bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20'
  },
  { 
    value: 'lost', 
    label: 'Cancelled', 
    ...
    btnSelectedClass: 'bg-red-600 text-white border border-red-600',
    btnActionClass: 'bg-red-500/10 text-red-500 hover:bg-red-500/20'
  },
  { 
    value: 'closed', 
    label: 'Closed', 
    ...
    btnSelectedClass: 'bg-slate-600 text-white border border-slate-600',
    btnActionClass: 'bg-slate-500/10 text-slate-500 hover:bg-slate-500/20'
  },
];
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <h3 className="font-bold text-slate-900 text-lg">{lead.listingName}</h3>
                    </div>
                    <p className="text-sm text-slate-500 ">{lead.consumerName} • {lead.projectLocation}</p>
                  </div>
                  <span className={`px-3 py-1 rounded-lg text-xs font-bold uppercase ${getStatusBadgeClass(lead.status)}`}>
                ...
                <div className="flex gap-6 text-sm">
                  <div>
                    ...
                    <p className="text-slate-900 font-medium">{lead.duration}</p>
                  </div>
                  <div className="flex-1">
                    <p className="text-slate-500 text-xs uppercase tracking-widest">Scope</p>
                    <p className="text-slate-900 font-medium truncate">{lead.scopeOfWork}</p>
                  </div>
                </div>
              </div>
            ))
        ...
        <div className="lg:col-span-1">
          {selectedLead ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-6 sticky top-8 shadow-sm">
              <h3 className="text-lg font-black text-slate-900 mb-6">Lead Details</h3>
              
              <div className="space-y-4 mb-6">
                <div>
                  <p className="text-xs text-slate-500 uppercase tracking-widest mb-1">Equipment</p>
                  <p className="text-slate-900 font-bold">{selectedLead.listingName}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 uppercase tracking-widest mb-1">Customer</p>
                  <p className="text-slate-900 font-bold">{selectedLead.consumerName}</p>
                  <p className="text-sm text-slate-500 ">{selectedLead.consumerEmail}</p>
                  <p className="text-sm text-slate-500 ">{selectedLead.consumerPhone}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 uppercase tracking-widest mb-1">Project Location</p>
                  <p className="text-slate-900 font-medium">{selectedLead.projectLocation}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 uppercase tracking-widest mb-1">Duration</p>
                  <p className="text-slate-900 font-medium">{selectedLead.duration}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 uppercase tracking-widest mb-1">Scope of Work</p>
                  <p className="text-slate-900 text-sm bg-slate-50 p-3 rounded-lg border border-slate-200 ">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {STATUS_OPTIONS.filter(s => s.value !== selectedLead.status).map(opt => (
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg text-xs font-bold transition-all ${opt.btnActionClass}`}
                    >
                      {opt.actionLabel}
                    </button>
                  ))}
